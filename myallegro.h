...
void initAllegro(){ 
	...
	install_keyboard();
    set_color_depth(32);  
	set_window_title("Samuele Siciliano & Saverio Crea APSD Project");
...
void drawWithAllegro(int stepCurr){  //funzione per la stampa a schermo tramite allegro
	...
	textout_ex(screen, font, str, 0, endY+1, makecol(255, 255, 255), -1); //+1 per farlo partire poco dopo la fine della matrice
